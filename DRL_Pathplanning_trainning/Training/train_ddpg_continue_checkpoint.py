"""
Continue training DDPG from a checkpoint WITHOUT loading the old replay buffer.

Usage::

    python Training/train_ddpg_continue_checkpoint.py \\
        --checkpoint Data/Training/DDPG/.../checkpoint_t5000000.zip \\
        --timesteps 5000000
"""
...
# --------------------------------------------------------------------------- #
# A fresh replay buffer after a long checkpoint means the rolling success-rate
# window is not yet meaningful.  Disable by default.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Continue DDPG training from checkpoint")
...
def _build_algo_cfg(cfg: Config) -> dict:
    ...
    return {
        "gamma": ddpg_c.gamma,
        "learning_rate": ddpg_c.learning_rate,
        "batch_size": ddpg_c.batch_size,
        "buffer_size": ddpg_c.buffer_size,
        "learning_starts": ddpg_c.learning_starts,
        "tau": ddpg_c.tau,
        "train_freq": ddpg_c.train_freq,
        "gradient_steps": ddpg_c.gradient_steps,
        "policy_kwargs": ddpg_c.policy_kwargs,
        "policy": ddpg_c.policy,
        ...
        } if cfg.action_noise.enabled else None,
    }
...
def _create_run_dir(suffix: str) -> TrainingRun:
    ...
    run_dir = base_dir / "DDPG" / "FRAME_ONLY" / f"run_{run_id}"
...
def _auto_suffix_from_checkpoint(checkpoint_path: Path) -> str:
    """Extract timestep tag from checkpoint filename, e.g. checkpoint_t25700000.zip → continue_from_t25700000_no_replay"""
    import re
...
def main() -> None:
    ...
    _test_model = DDPG.load(str(args.checkpoint), env=gym_env, device=device)
    ...
    model = create_sb3_model(algorithm="DDPG", env=gym_env, algo_cfg=algo_cfg, device=device)
    ...
    print("=" * 60)
    print(f"  DDPG — Continue from checkpoint")
    print("=" * 60)
