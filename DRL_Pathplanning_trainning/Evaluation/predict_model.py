"""
TD3 model evaluation script — loads from config/environment.yaml.

Usage::

    # Default: uses config/environment.yaml + Data/Training/TD3/FRAME_ONLY/run
    python Evaluation/predict_model.py --episodes 10 --gui false --show false

    # With custom run dir
    python Evaluation/predict_model.py \\
        --config config/environment.yaml \\
        --run Data/Training/TD3/FRAME_ONLY/run \\
        --episodes 10 \\
        --mode random

    # Custom checkpoint
    python Evaluation/predict_model.py \\
        --model checkpoint_t15000000.zip \\
        --episodes 10
"""
...
DEFAULT_RUN_DIR = PROJECT_ROOT / "Data" / "Training" / "TD3" / "FRAME_ONLY" / "run_20260501_random_obs_start_size"

CUSTOM_MODEL_ZIP = "checkpoint_t138627720.zip"  # e.g. "checkpoint_t15000000.zip"
# --------------------------------------------------------------------------- #
...
def main() -> int:
    ...
    print(f"[INFO] Env workspace   : z_max={float(cfg.workspace.max_np[2]):.3f}m")

    # ---- Load model ----
    model = TD3.load(str(model_path), env=None, device=DEVICE)
    ...
    output_dir = (
        ...
        / f"Environment_Default" / ALGORITHM / "FRAME_ONLY"
    )
