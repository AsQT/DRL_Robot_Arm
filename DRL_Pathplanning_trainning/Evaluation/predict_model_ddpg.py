"""
DDPG model evaluation script — loads from config/environment.yaml.

Usage::

    python Evaluation/predict_model_ddpg.py --episodes 10 --gui false --show false

    python Evaluation/predict_model_ddpg.py \\
        --config config/environment.yaml \\
        --run Data/Training/DDPG/FRAME_ONLY/run_xxx \\
        --episodes 10 \\
        --mode random"""
...
DEFAULT_RUN_DIR = PROJECT_ROOT / "Data" / "Training" / "DDPG" / "FRAME_ONLY"

CUSTOM_MODEL_ZIP = ""  # e.g. "checkpoint_t5000000.zip"
# --------------------------------------------------------------------------- #
...
def main() -> int:
    ...
    model = DDPG.load(str(model_path), env=None, device=DEVICE)
