...
Fine-tune an existing model::
    ...
    python Training/train_td3.py --config config/environment.yaml \\
        ...
        --load-vecnormalize Data/Training/.../vec_normalize_stats.pkl
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to vec_normalize_stats.pkl to load for fine-tuning",
    )
    parser.add_argument(
        "--load-replay-buffer",
        type=Path,
...
def main() -> None:
    ...
    if algo not in ("TD3", "DDPG", "SAC"):
        sys.exit(
            f"[ERROR] Unknown algorithm '{algo}' in config. "
            f"Supported: TD3, DDPG, SAC"
        )
