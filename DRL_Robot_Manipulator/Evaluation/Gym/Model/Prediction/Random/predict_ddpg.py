"""
Random-target prediction script for DDPG.
...
CONST_MODEL_PATH = r"Data/Training/Environment_Default/DDPG/YASKAWA_GP7/run_20260506_113510/model/final_model.zip"
...
CONST_DISTANCE_THRESH = 0.05   # must match training (0.01 m)
CONST_MAX_STEPS = 200
CONST_NUM_EPISODES = 100
CONST_SEED = 42
CONST_VERBOSE_EPISODE = True   # print per-episode summary

# --- Output ---
# Path to .txt file with per-episode results.
# None = auto-generated at Data/Prediction/Environment_{mode}/{algo}/YASKAWA_GP7/
...
def _resolve_model_path(model_path, run_dir, algorithm):
    ...
    def _resolve(p: str, base: Path) -> Path:
        """Resolve a string path as absolute, or relative to base."""
        p = Path(p)
        if p.is_absolute():
            return p
    ...
    for ep_idx in range(1, CONST_NUM_EPISODES + 1):
        ...
        while True:
            ...
            if terminated or truncated:
                ...
                # Log: [episode_index, is_success, cumulative_reward, episode_length, final_distance]
