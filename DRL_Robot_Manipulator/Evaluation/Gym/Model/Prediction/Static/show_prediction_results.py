...
def main():
    ...
    for i, (P_i, S_i, p_i) in enumerate(zip(S_Cls.P.T, S.T, T.p.all())):
        # Create a figure.
        _, ax = plt.subplots()

        # Visualization of relevant structures.
        ax.scatter(t_hat[-1], p_i, c='#84b070', s=100.0, linewidths=1, label=f'Target')
        ax.plot([t_hat[-1]] * 2, [P_i[-1], p_i], '.-', color='#e06666', linewidth=1.0, label=f'$\Delta$e')
        ax.plot(t_hat, P_i, '.--', color='#8d8d8d', alpha=1.0, markersize=8.0, markeredgewidth=2.0, markerfacecolor='#ffffff',
                label=f'Control Points')
        ax.plot(S_Cls.x, S_i, '-', color='#8d8d8d', linewidth=1.0, markerfacecolor='#8d8d8d', 
                label=f'B-Spline')
        ...
        #   Set the y ticks.
        tick_y_tmp = (np.max(P_i) - np.min(P_i))/10.0
        tick_y = tick_y_tmp if tick_y_tmp != 0.0 else 0.1
        ax.set_yticks(np.arange(np.min(P_i) - tick_y, np.max(P_i) + tick_y, tick_y))
        #   Label.
