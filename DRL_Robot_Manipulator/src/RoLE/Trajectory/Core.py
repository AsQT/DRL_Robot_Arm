"""
RoLE Multi-Segment Trajectory Generation
=====================================
Multi-segment trajectory through a list of waypoints with configurable blend phases.
Each segment uses either a trapezoidal or polynomial profile from RoLE.Trajectory.Utilities.
Supports arc-length parameterisation and cumulative time computation.
"""
...
class Multi_Segment_Cls(object):
    ...
    def __Generate_Trapezoidal(self, P: tp.List[tp.List[float]], t_blend: tp.List[float], T: tp.List[float], 
        ...
        # profile.
        T_Cls = RoLE.Trajectory.Utilities.Trapezoidal_Profile_Cls(self.__delta_time)
    ...
    def __Generate_Polynomial(self, P: tp.List[tp.List[float]], t_blend: tp.List[float], T: tp.List[float], 
        ...
        L_Cls = RoLE.Trajectory.Utilities.Linear_Function_Cls(self.__delta_time)

        # Initialization of the class to generate trajectory using using a polynomial 
        # profile of degree 5 (quintic).
        P_Cls = RoLE.Trajectory.Utilities.Polynomial_Profile_Cls(self.__delta_time)
