...
"""
RoLE Kinematics Utilities — General Helpers
=====================================
General kinematics helpers: joint limit validation, 6-D error vector computation,
quadratic error with weights, singularity detection via SVD, self/external collision
checking, and IK solution selection.

Used by the kinematics core layer.
"""
...
def Get_Best_IK_Solution(theta_0: tp.List[float], theta_solutions: tp.List[tp.List[float]], Robot_Parameters_Str: Parameters.Robot_Parameters_Str) -> tp.List[float]:
    ...
    for _, th_sol_i in enumerate(theta_solutions):
        ...
        (th_limit_err, _) = RoLE.Kinematics.Core.Forward_Kinematics(th_sol_i, 'Modified', Robot_Parameters_Str)
