...
"""
RoLE Kinematics — Forward and Inverse Kinematics
============================================
Forward kinematics (FK) and numerical inverse kinematics (IK) for serial-chain
manipulators using Denavit-Hartenberg parameters.

Supports multiple IK solvers: Jacobian Transpose, Newton-Raphson, Gauss-Newton,
and Levenberg-Marquardt. Includes geometric Jacobian computation, self-collision
and external-collision checks, and singularity detection.
"""
...
def Get_Geometric_Jacobian(theta: tp.List[float], Robot_Parameters_Str: Parameters.Robot_Parameters_Str) -> tp.List[tp.List[float]]:
    ...
    # Standard forward kinematics calculation method.
    #T_Cfg_Arr = __Get_Individual_Joint_Configuration_Standard(th, Robot_Parameters_Str)
...
def Inverse_Kinematics_Numerical(TCP_Position: tp.List[tp.List[float]], theta_0: tp.List[float], method: str, 
    ...
    try:
        ...
        T_0 = Forward_Kinematics(theta_0, 'Modified', Robot_Parameters_Str)[1]
        ...
        for _, t_i in enumerate(t):
            ...
            for iteration_i in range(ik_solver_properties['num_of_iteration']):
                ...
                T = Forward_Kinematics(th_i, 'Modified', Robot_Parameters_Str)[1]
