"""
Soft Actor-Critic (SAC) training script for the Yaskawa GP7 robot.

Trains a SAC agent on the YaskawaGP7ReachPyBullet Collision-Free environment,
...
SAC uses entropy-based exploration and does not require an explicit action noise
model.  It is well-suited for continuous control tasks with exploration challenges.
...
# from both "python Training/train_sac.py" and "cd Training && python train_sac.py".
...
Notes:
    ...
    $ cd Training && python train_sac.py
"""
...
def main() -> None:
    ...
    SAC model, trains for CONST_TOTAL_TIMESTEPS, saves the final policy and
    ...
    # Build model (SAC uses entropy-based exploration; no action noise needed).
